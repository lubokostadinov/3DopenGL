...
private:
	...
	//Private functions
	std::string loadShaderSourcse( const char* fileName)
	{
		std::string temp = "";
		std::string src = "";
		...
		std::string versionNr =
			...
			std::to_string(this->versionMinor) + "0";
		src.replace(src.find("#version"), 12, ("#version " + versionNr));
		std::cout << fileName << " #version " + versionNr << "\n";
	...
	GLuint loadShader(GLenum type,const char* fileName)
	{
		...
		std::string str_src = this->loadShaderSourcse(fileName);
